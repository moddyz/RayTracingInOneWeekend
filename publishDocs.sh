...
git checkout master
rm -rfv ./build
mkdir -p /tmp/RayTracingInOneWeekend
rm -rfv /tmp/RayTracingInOneWeekend
./build.sh /tmp/RayTracingInOneWeekend

# Checkout the gh-pages branch, remove all files, and copy the
# installed documentation into the repo root.
git checkout gh-pages
rm -rf *
cp -r /tmp/RayTracingInOneWeekend/docs/doxygen_docs/html/* ./
